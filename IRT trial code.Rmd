...
c. Estimate RSME for data generation and estimation for 1PL, 2PL and 3PL - *I can't replicate results from paper here*
d. Run 50 simulations to see which model gives the best item parametrics
...
4. Estimate parameters for each of the datasets, using the correct model (i.e. estimate b for 1PL, a&b for 2PL and a&b&c for 3PL). Estimated parameters are names a_est, b_est, c_est

5. Compare estimations using RSME

#### Issue faced - 
1. The paper said that 1PL always performs better according to RSME, but for me, 2PL performs better
2. The paper mentioned mean-mean linking - "Before calculating RMSEs, the estimated parameters were linked to the generating parameter scale using the mean-mean procedure (pg 7)". I am not sure what this means
...
1. I generate parameters (a,b,c) only once. Using this, data is generated. Using generated datasets, parameters are estimated for 50 iterations, and we see which model performed best. 
...
# Create vectors to record final summary
	for (i in 1:iter){
dat_1PL = fun_simulate_data(40, 1000, "1PL", a1, b,c1, low_ability)
...
df_AIC$best_model_AIC = row.names(df_AIC)

df_BIC=data.frame(BIC_count=unclass(table(bestfit_1PL$BIC)))
df_BIC$best_model_BIC = row.names(df_BIC)

results1 = cbind(DGM="1PL",df_AIC, df_BIC)
...
results2 = cbind(DGM="2PL",df_AIC, df_BIC)
...
# fit = apply(bestfit_3PL,MARGIN=2,table)
# 
# summary_3PL = cbind(DGM= "3PL", 
#                     AIC=fit$AIC[[1]],
#                     BIC=fit$BIC[[1]],
#                     LL=fit$LL[[1]])
# 
# summary= rbind(summary, summary_3PL)
...
results3 = cbind(DGM="3PL",df_AIC, df_BIC)

rbind(results1,results2,results3)
